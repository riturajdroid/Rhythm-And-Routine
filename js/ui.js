...
// CHANGED: Removed stepcount array as requested
...
newTrack.addEventListener('click', () => {
  // CHANGED: Increment numberOfTracks exactly once here to prevent ID mismatches throughout the block
  numberOfTracks++; 

  const newTrackObj = { // CHANGED: Renamed variable to avoid shadowing the button variable
    id: `track-${numberOfTracks}`, // CHANGED: Removed ++ since it's incremented above
    steps: 4,
    pattern: [0, 0, 0, 0],
    volume: 0.6,
    mute: false,
  ...
  // CHANGED: Removed stepcount.push(4)
...
mainbody.addEventListener('click', (e) => {
  ...
  if (e.target.classList.contains('plus')) {
    ...
    const trackIndex = trackID - 1; // CHANGED: Mapped 1-based ID to 0-based tracks array index
    
    if (tracks[trackIndex].steps > 9) return; // CHANGED: Replaced stepcount with tracks[].steps
    tracks[trackIndex].steps++; // CHANGED: Replaced tracks[trackID] with correct trackIndex
    tracks[trackIndex].pattern.push(0); // CHANGED: Expand pattern array to match new steps
    // CHANGED: Removed stepcount[trackID]++

    const stepDesign = document.getElementById(`step-design-track-${trackID}`);
    let tilesHTML = '';
    for (let index = 0; index < tracks[trackIndex].steps; index++) {
      // CHANGED: Replaced global numberOfTracks with trackID, and index + 1 with index to fix ID mismatches
    ...
    if (stepCountText) {
      stepCountText.textContent = tracks[trackIndex].steps;
    }
  }
  ...
  if (e.target.classList.contains('minus')) {
    ...
    if (tracks[trackIndex].steps < 2) return; // CHANGED: Replaced stepcount with tracks[].steps
    tracks[trackIndex].steps--; // CHANGED: Replaced tracks[trackID] with correct trackIndex
    tracks[trackIndex].pattern.pop(); // CHANGED: Shrink pattern array to match removed steps
    // CHANGED: Removed stepcount[trackID]--
    ...
    if (stepCountText) {
      stepCountText.textContent = tracks[trackIndex].steps; // CHANGED: Replaced stepcount
    }
  }
  ...
  if (e.target.classList.contains('muteButton')) {
    ...
    if (e.target.style.backgroundColor === "") {
      ...
      tracks[trackIndex].mute = true; // CHANGED: Replaced tracks[trackID] with trackIndex
      soloBtn.style.backgroundColor = "";
      soloBtn.style.scale = 1;
      tracks[trackIndex].solo = false; // CHANGED: Replaced tracks[trackID] with trackIndex
    } else {
      e.target.style.backgroundColor = "";
      e.target.style.scale = 1;
      tracks[trackIndex].mute = false; // CHANGED: Replaced tracks[trackID] with trackIndex
    }
  }
  ...
  if (e.target.classList.contains('soloButton')) {
    ...
    const trackID = e.target.id.split('-').pop(); // CHANGED: Extracted missing trackID in this block
    const trackIndex = trackID - 1; // CHANGED: Mapped 1-based ID to 0-based tracks array index

    if (e.target.style.backgroundColor === "") {
      e.target.style.backgroundColor = "red";
      e.target.style.scale = 1.3;
      tracks[trackIndex].solo = true; // CHANGED: Replaced tracks[trackID] with trackIndex
  ...
  if (e.target.classList.contains('tile')) {
    ...
    if (numbers && numbers.length >= 2) {
        ...
        const trackIndex = trackNumber - 1; // CHANGED: Mapped 1-based ID to 0-based tracks array index
        const stepIndex = parseInt(numbers[1], 10);
        const isActive = e.target.classList.contains('active');
        tracks[trackIndex].pattern[stepIndex] = isActive ? 1 : 0; // CHANGED: Used trackIndex instead of trackNumber directly
    ...
    input.addEventListener('change', (e) => {
        ...
        reader.onload = (event) => {
            ...
            data.tracks.forEach(savedTrack => {
                ...
                // render tiles with saved active state
                const stepDesign = document.getElementById(`step-design-track-${numberOfTracks}`);
                let tilesHTML = '';
...
document.addEventListener('keydown', (e) => {
    if (e.code === 'Space') {
        e.preventDefault(); // stops page from scrolling on spacebar
        playPauseBtn.click();
    }
});
...
function animatePlayhead() {
    ...
    eventQueue.forEach(event => {
        if (event.time <= now) {
            // clear previous active playhead on this track
